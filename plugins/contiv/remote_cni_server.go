...
func (s *remoteCNIserver) configurePodInterface(request *cni.CNIRequest, podIP net.IP, config *PodConfig,
	txn linux.PutDSL, revertTxn linux.DeleteDSL) error {
...
func (s *remoteCNIserver) unconfigurePodInterface(request *cni.CNIRequest, config *container.Persisted,
	...
	if !s.test {
		...
		txn.LinuxArpEntry(config.PodARPEntryName)
	}
...
// unconfigurePodVPPSide prepare transaction <txn> to delete vswitch VPP part of the POD networking.
func (s *remoteCNIserver) unconfigurePodVPPSide(config *container.Persisted, txn linux.DeleteDSL) error {
